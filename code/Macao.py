...
class Graph:
	...
	def observe(self, I):
		...
		obs = np.zeros(self.N)
		for i in range(0, self.num_cluster):
			obs[self.cluster_bounds[i]:self.cluster_bounds[i + 1]] = np.random.choice(2, size=self.cluster_size[i],
																					  p=[1 - self.r_mat[c, i],
																						 self.r_mat[c, i]])
		obs[I] = 1
		return obs
	...
	def choose_arm(self, observe, loss):
		...
		# TODO perform estimation
			...
			eta = np.sqrt(np.log(self.N) / ((self.N * self.N)
											+ np.sum(
				np.multiply(np.multiply(self.p[e:t:2], self.DLhat[e:t:2]), self.DLhat[e:t:2]))))
			w = -eta * self.Lhat[t - 2, :]
			w = w - np.max(w)
			w = np.exp(w)
			self.p[t, :] = w / np.sum(w)
